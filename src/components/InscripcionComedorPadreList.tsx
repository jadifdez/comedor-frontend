...
interface InscripcionComedorPadreListProps {
  inscripciones: InscripcionPadre[];
  onDesactivar: (id: string) => void;
  calcularPrecioMensual: (inscripcion: InscripcionPadre) => number;
  nombrePadre: string;
}

const diasSemanaLabels = ['', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes'];

export function InscripcionComedorPadreList({
  inscripciones,
  onDesactivar,
  calcularPrecioMensual,
  nombrePadre
}: InscripcionComedorPadreListProps) {
  const [showDeleteModal, setShowDeleteModal] = React.useState(false);
  const [inscripcionToDelete, setInscripcionToDelete] = React.useState<InscripcionPadre | null>(null);
  const [expandedInactivas, setExpandedInactivas] = React.useState<Set<string>>(new Set());

  const handleDeleteClick = (inscripcion: InscripcionPadre) => {
  ...
  if (inscripciones.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">No tienes inscripciones al comedor</h3>
        <p className="text-gray-600">Tus inscripciones al comedor aparecerán aquí</p>
      </div>
    );
  }

  const renderInscripcion = (inscripcion: InscripcionPadre, isActive: boolean) => {
    if (!isActive) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-gray-900 flex items-center space-x-2">
        ...
        <span>Mis inscripciones al comedor ({inscripciones.length})</span>
      </h3>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
            ...
            <div className="mb-6">
              <p className="text-gray-600 mb-2">
                ¿Estás seguro de que quieres desactivar tu inscripción al comedor?
              </p>
              {inscripcionToDelete && (
                <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                  <div className="font-medium text-gray-900">{nombrePadre}</div>
