/*
  # Create optimized RPC function for personal staff listing

  1. New Function
    - `get_personal_with_counts` - Returns personal staff with hijo counts and inscription status
    - Replaces multiple separate queries with a single optimized query
    - Significantly improves performance for PersonalManager component
...
-- Create optimized function to get personal staff with counts
...
BEGIN
  ...
  GROUP BY p.id, p.email, p.nombre, p.telefono, p.activo, p.created_at, 
